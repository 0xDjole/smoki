...
const createNavigateStore = () => {
    ...
    const back = () => update((prevState) => {
        ...
        if (basePath) {
            ...
            if (navHistory && navHistory.length > 1) {
                ...
                const backScreen = navScreens.find((screen) => screen.path === parentNavigator.screens[lastHistoryScreen].backDefault);
    ...
    return {
        ...
        init: (screens, navigationType, urlObject, navigationPath, defaultIndex) => update((prevState) => {
            ...
            const parsedScreens = screens.map((screen, index) => {
                ...
                return {
                    ...
                    index,
                    opened: navigationType === 'Stack' || navigationType === 'Drawer'
                        ? true
        ...
        navigate: (url, { reload } = { reload: false }, callback = () => { }) => update((prevState) => {
            const navigationPath = prevState.navigationPath[window.location.pathname];
            if (prevState.navigating) {
                return prevState;
            }
            ...
            wantedNavigationPath.forEach((wantedPathItem, index) => {
                if (navigationPath[index]) {
                    if (wantedPathItem.index !== navigationPath[index].index) {
                        ...
                        const navType = wantedNavScreen.navigationPath[wantedNavScreen.navigationPath.length - 1]
                            .navigationType;
                        if (navType === 'Stack') {
                            for (const [key, value] of urlObject.searchParams) {
                                urlObject.searchParams.set(key, value);
                            ...
                            for (let i = 0; i < navScreens.length; i++) {
                                ...
                                if (i === wantedScreenIndex) {
                                    ...
                                    while (currentScreen?.navigation?.history?.length) {
                                        const lastHistoryIndex = currentScreen.navigation.history[currentScreen.navigation.history.length - 1];
